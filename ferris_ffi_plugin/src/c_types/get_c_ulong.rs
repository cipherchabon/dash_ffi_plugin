use std::{
    alloc::{alloc, Layout},
    ffi::{c_char, c_ulong, c_ulonglong, CString},
    mem,
};
...
// ulong cannot be represented using Dart's int type. Please use the
// `get_max_c_ulong_as_bytes` function instead, which returns the maximum
// value as a byte array that can be safely converted to Dart's BigInt.
#[no_mangle]
pub extern "C" fn get_max_c_ulong() -> c_ulong {
    c_ulong::max_value()
}

#[no_mangle]
pub extern "C" fn get_min_c_ulong() -> c_ulong {
    c_ulong::min_value()
}
...
#[no_mangle]
pub extern "C" fn get_max_c_ulong_as_string() -> *mut c_char {
...
#[no_mangle]
pub extern "C" fn get_max_c_ulong_as_bytes() -> ByteArray {
    let max_value = c_ulong::max_value();
